...
export function PublicShell({ publication, pages = [], children }: { publication?: PublicationInfo | null; pages?: PublicPageInfo[]; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-background"><a href="#main" className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-50 focus:rounded-md focus:bg-primary focus:px-4 focus:py-2 focus:text-primary-foreground">Skip to content</a>
    <header className="sticky top-0 z-40 border-b border-border/80 bg-background/90 backdrop-blur-md supports-[backdrop-filter]:bg-background/80">
      ...
      {open && <div className="fixed inset-0 z-50 flex flex-col bg-background md:hidden" role="dialog" aria-modal="true" aria-label="Site menu"><div className="container flex h-16 items-center justify-between gap-5"><Link href="/" className="flex items-center gap-4" aria-label={`${siteName} home`} onClick={() => setOpen(false)}>{mark(false, "h-[29px] w-[29px]")}<span className="font-display text-2xl font-semibold tracking-tight">{siteName}</span></Link><Button variant="ghost" size="icon" aria-label="Close menu" onClick={() => setOpen(false)}><X className="h-5 w-5" /></Button></div><nav className="container flex flex-1 flex-col justify-center gap-1 overflow-y-auto py-6" aria-label="Mobile navigation">{navigation.map((item, index) => <a key={`${item.label}-${item.path}`} href={item.path} onClick={() => setOpen(false)} className="flex items-center gap-4 border-b border-border/60 py-4 font-display text-3xl font-semibold tracking-tight transition-colors hover:text-primary"><span className="font-label text-[11px] text-muted-foreground">0{index + 1}</span>{item.label}</a>)}{adminUrl && <Link href={adminUrl} onClick={() => setOpen(false)} className="flex items-center gap-2 py-4 text-lg font-medium text-primary">Open Studio <ArrowUpRight className="h-5 w-5" /></Link>}</nav><div className="container pb-10"><a href="#newsletter" onClick={() => setOpen(false)}><Button className="h-12 w-full gap-2 text-base">Subscribe <ArrowUpRight className="h-4 w-4" /></Button></a></div></div>}
    </header>
