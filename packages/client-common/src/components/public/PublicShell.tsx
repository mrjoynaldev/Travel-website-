  ...
  return <div className="min-h-screen bg-background"><a href="#main" className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-50 focus:rounded-md focus:bg-primary focus:px-4 focus:py-2 focus:text-primary-foreground">Skip to content</a>
    <header className="sticky top-0 z-40 border-b border-border/80 bg-background/90 backdrop-blur-md">
      <div className="container flex h-16 items-center justify-between gap-5">
        <Link href="/" className="flex items-center gap-4" aria-label={`${siteName} home`}>{mark(false, "h-[29px] w-[29px]")}<span className="font-display text-2xl font-semibold tracking-tight">{siteName}</span></Link>
        ...
        <div className="hidden items-center gap-2 md:flex">{adminUrl && <Link href={adminUrl}><Button variant="ghost" size="sm" className="gap-2"><Sparkles className="h-4 w-4" />Write</Button></Link>}<a href="#newsletter"><Button size="sm" className="gap-2">Subscribe <ArrowUpRight className="h-4 w-4" /></Button></a></div>
      ...
      {open && <nav className="border-t border-border bg-background px-4 py-4 md:hidden" aria-label="Mobile navigation"><div className="flex flex-col gap-2">{navigation.map(item => <a key={`${item.label}-${item.path}`} href={item.path} onClick={() => setOpen(false)} className="rounded-md px-3 py-2 hover:bg-muted">{item.label}</a>)}{adminUrl && <Link href={adminUrl} onClick={() => setOpen(false)} className="rounded-md px-3 py-2 text-primary hover:bg-muted">Open Studio</Link>}</div></nav>}
    </header>
    <main id="main">{children}</main>
    <footer id="about" className="border-t border-border bg-[#1b382d] text-[#edf4ea]"><div className="container grid gap-10 py-14 md:grid-cols-[1.2fr_.8fr_.8fr]"><div><div className="flex items-center gap-3">{mark(true)}<span className="font-display text-2xl font-semibold">{siteName}</span></div><p className="mt-5 max-w-sm text-sm leading-6 text-[#c6d5ca]">{brand.tagline || publication?.description || "Developer-first AI news, analysis, and practical guides for people who build and ship software."}</p></div><div><p className="font-label text-xs text-[#a7c0af]">Explore</p><div className="mt-4 flex flex-col gap-2 text-sm"><Link href="/" className="hover:text-[#e4a741]">Latest stories</Link><a href="/#topics" className="hover:text-[#e4a741]">Browse topics</a><Link href="/archive" className="hover:text-[#e4a741]">Publication archive</Link>{footerLinks.map(item => <a key={`${item.label}-${item.path}`} href={item.path} className="hover:text-[#e4a741]">{item.label}</a>)}</div></div><div><p className="font-label text-xs text-[#a7c0af]">Publishing</p><p className="mt-4 text-sm leading-6 text-[#c6d5ca]">{contact.name || "Clear-eyed reporting and practical guidance on the AI landscape, built to respect your attention."}</p>{contact.email && <a href={`mailto:${contact.email}`} className="mt-3 block text-sm text-[#e4a741] hover:underline">{contact.email}</a>}</div></div><div className="border-t border-[#396550] py-5 text-center text-xs text-[#a7c0af]">© {new Date().getFullYear()} {siteName}. Developer-first AI news & analysis.</div></footer>
  </div>;
}
