...
const FALLBACK_EMAIL = "hello@sundarbanyatri.com";
...
export function PublicShell({ publication, pages = [], children }: { publication?: PublicationInfo | null; pages?: PublicPageInfo[]; children: React.ReactNode }) {
  ...
  // Footer "Explore" has fixed static links; admin-configured footer_links are
  // appended only when they point somewhere new. Without this, an admin link
  // to /tours (etc.) renders the same destination twice. Matching is by
  // normalized path so labels can differ ("Tours" vs "Sundarban Tours").
  const normPath = (p: string) => (p || "").trim().replace(/\/+$/, "") || "/";
  const STATIC_EXPLORE_PATHS = new Set(["/tours", "/tours/sundarban-winter-festival", "/tours/sundarban-hilsa-festival", "/archive"]);
  const footerLinks = (settings.footer_links?.length ? settings.footer_links : pages.map(page => ({ label: page.title, path: `/${page.slug}` })))
    .filter(item => !STATIC_EXPLORE_PATHS.has(normPath(item.path)));
  // The mobile menu hardcodes an "All Guides" entry; skip it when the stored
  // navigation already links the archive (e.g. a "Guides" item).
  const hasArchiveNav = navigation.some(item => normPath(item.path) === "/archive");
    ...
    <div className="hidden bg-[#0f4532] text-[#cfe0d5] md:block">
      <div className="container flex h-9 items-center justify-between text-xs">
        ...
        <div className="flex items-center gap-4">
          <a href={waLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 hover:text-white transition-colors"><WhatsAppIcon className="h-3.5 w-3.5" /> WhatsApp</a>
      ...
      <div className="container grid gap-10 py-14 lg:py-16 md:grid-cols-2 lg:grid-cols-[1.3fr_.7fr_.7fr_.8fr]">
        <div>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <a href={waLink} target="_blank" rel="noopener noreferrer" className="inline-flex h-10 items-center gap-2 rounded-full bg-[#1fa855] px-4 text-sm font-semibold text-white"><WhatsAppIcon className="h-4 w-4" /> WhatsApp</a>
