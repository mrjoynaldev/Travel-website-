...
# Usage:  mkdir -p ~/crg-cli && cd ~/crg-cli && curl -fsSL https://codereportglobal.indevs.in/docs/setup.sh -o setup.sh && bash setup.sh
set -e

BASE="https://codereportglobal.indevs.in/docs"
DIR="$(pwd)"
...
Next steps (as documented in https://codereportglobal.indevs.in/docs/AI-EDITOR-AGENT.md):
