...
export default function ArticleView({ post, related, comments: initialComments, slug }: { post: Post; related: ArticleCardPost[]; comments: Comment[]; slug: string }) {
  ...
  return <>
    <article>
      ...
      <div className="container max-w-3xl pb-4">{post.featuredMedia && <figure className="mb-10"><img src={post.featuredMedia.url} alt={post.featuredMedia.alt_text || ""} className="aspect-[16/9] w-full rounded-2xl object-cover" />{post.featuredMedia.caption && <figcaption className="mt-2 text-xs text-muted-foreground">{post.featuredMedia.caption}</figcaption>}</figure>}<div className="article-prose" dangerouslySetInnerHTML={{ __html: bodyHtml }} /><div className="mt-12 flex flex-wrap gap-2 border-t border-border pt-7">{post.tags.map(tag => <span key={tag.id} className="rounded-full border border-border bg-white px-3 py-1 text-xs text-muted-foreground">#{tag.name}</span>)}</div><section className="mt-14 rounded-2xl bg-white p-6 shadow-sm"><div className="flex gap-4"><Avatar className="h-12 w-12"><AvatarFallback className="bg-primary text-primary-foreground">{authorInitials}</AvatarFallback></Avatar><div>{post.author ? <Link href={`/authors/${post.author.id}`} className="text-sm font-semibold hover:underline">Written by {post.author.display_name}</Link> : <p className="text-sm font-semibold">Written by CodeReport Global editorial</p>}{post.author?.bio && <p className="mt-1 text-sm leading-6 text-muted-foreground">{post.author.bio}</p>}</div></div></section></div>
    </article>
