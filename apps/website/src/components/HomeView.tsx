...
export default function HomeView({ categories, sections, posts, search, category, page }: HomeViewProps) {
  ...
  return <>
    <section className="paper-grid border-b border-border"><div className="container grid gap-10 py-16 md:grid-cols-[1.1fr_.9fr] md:py-24"><div className="enter-fade"><p className="font-label text-xs text-primary">Fix dev errors fast with AI — 18, AI-native</p><h1 className="mt-5 max-w-3xl font-display text-5xl font-semibold leading-[.98] tracking-tight text-foreground sm:text-6xl lg:text-7xl">I fix <em className="font-normal text-primary">dev errors</em><br />in minutes.</h1><p className="mt-6 max-w-lg text-base leading-7 text-muted-foreground">CodeReport Global: practical fix guides + AI builds. Stuck on a bug, need a landing page in 24h, or want an automation? I’ll ship it — no fluff. <Link href="/hire" className="font-medium text-primary underline">Hire me →</Link></p><div className="mt-8 flex flex-wrap gap-3"><Button asChild><Link href="/hire">Get help in 24h</Link></Button><Button variant="outline" asChild><Link href="/archive">Browse guides</Link></Button></div><div className="mt-6 max-w-md"><SearchField value={searchInput} onChange={onSearchChange} /></div></div><div className="flex flex-col justify-end rounded-2xl border border-[#cbd8c5] bg-[#e4ece0]/80 p-7 shadow-[0_18px_40px_-32px_rgba(31,77,59,.55)]"><p className="font-label text-[10px] text-primary">The editorial note</p><p className="mt-5 font-display text-2xl leading-snug text-[#263c31]">We believe attention is a form of care. The best ideas have room to unfold — and then ship.</p><div className="mt-6 flex items-center gap-3 border-t border-[#c6d4c2] pt-4">
    ...
    <section className="container py-14 md:py-20">{showConfiguredSections ? <ConfiguredSections sections={sections} /> : <><div className="mb-10 flex items-end justify-between gap-4"><div><p className="font-label text-xs text-primary">From the journal</p><h2 className="mt-2 font-display text-4xl font-semibold tracking-tight">{heading}</h2></div><p className="hidden text-sm text-muted-foreground sm:block">{posts.total} published pieces</p></div>{isLoading ? <div className="grid min-h-64 place-items-center"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : posts.items.length ? <div className="space-y-14"><ArticleCard post={posts.items[0]} featured /><div className="grid gap-x-7 gap-y-12 border-t border-border pt-12 md:grid-cols-3">{posts.items.slice(1).map(post => <ArticleCard key={post.id} post={post} />)}</div><div className="mt-12 rounded-xl border border-border bg-[#fbfcfa] p-6"><p className="font-label text-xs text-primary">All guides — for readers and search engines</p><ul className="mt-3 space-y-2 pl-0 list-none">{posts.items.map(p => <li key={p.id} className="flex items-start gap-2 text-sm"><span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-primary" /><Link href={`/articles/${p.slug}`} className="font-medium text-primary hover:underline leading-6">{p.title}</Link>{p.excerpt && <span className="hidden text-muted-foreground sm:inline">— {p.excerpt.slice(0, 80)}…</span>}</li>)}</ul></div></div> : <div className="rounded-2xl border border-dashed border-border bg-white px-6 py-16 text-center"><p className="font-display text-2xl">There are no published stories here yet.</p><p className="mt-2 text-sm text-muted-foreground">Try a different search, explore another topic, or return soon.</p></div>}{posts.totalPages > 1 && <div className="mt-12 flex items-center justify-center gap-3"><Button variant="outline" disabled={page === 1} onClick={() => navigate({ page: page - 1 })}>Previous</Button><span className="text-sm text-muted-foreground">Page {page} of {posts.totalPages}</span><Button variant="outline" disabled={page === posts.totalPages} onClick={() => navigate({ page: page + 1 })}>Next</Button></div>}</>}</section>
