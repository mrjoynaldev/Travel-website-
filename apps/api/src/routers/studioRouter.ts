...
export const studioRouter = router({
  ...
  media: router({
    ...
    update: protectedProcedure.input(z.object({ id: z.string().uuid(), altText: z.string().max(300).optional(), caption: z.string().max(500).optional(), filename: z.string().max(180).optional() })).mutation(async ({ ctx, input }) => { const actor = await actorFor(ctx); assertRole(actor, ["admin", "editor"]); const updates: Record<string, unknown> = { updated_at: new Date().toISOString() }; if (input.altText !== undefined) updates.alt_text = input.altText || null; if (input.caption !== undefined) updates.caption = input.caption || null; if (input.filename !== undefined) updates.filename = input.filename; const { data, error } = await getSupabase().from("media_assets").update(updates).eq("id", input.id).eq("organization_id", actor.organizationId).eq("site_id", actor.siteId).select("*").single(); if (error || !data) throw new TRPCError({ code: "NOT_FOUND", message: "Media asset could not be updated." }); await recordAudit(actor, "media.updated", "media", data.id, { fields: Object.keys(updates) }); return data; }),
  ...
  automations: router({
    ...
    create: protectedProcedure.input(z.object({ platform: z.enum(["instagram"]), post_id: z.string().min(1).max(200), post_title: z.string().max(300).default(""), keyword: z.string().trim().max(500).optional(), keywords: z.string().trim().max(500).optional(), any_comment: z.boolean().default(false), dm_template: z.string().trim().min(1).max(1000), button_text: z.string().trim().max(30).default("Read Full Guide"), button_url: z.string().url().max(500), follow_required: z.boolean().default(true) })).mutation(async ({ ctx, input }) => { const actor = await actorFor(ctx); assertRole(actor, ["admin", "editor"]); const kw = (input.keywords || input.keyword || "").toLowerCase().trim(); const anyC = input.any_comment || (!kw); const { data, error } = await getSupabase().from("automations").insert({ organization_id: actor.organizationId, site_id: actor.siteId, platform: input.platform, post_id: input.post_id, post_title: input.post_title, keyword: kw.split(",")[0]?.trim() || kw, keywords: kw, any_comment: anyC, dm_template: input.dm_template, button_text: input.button_text, button_url: input.button_url, follow_required: input.follow_required }).select("*").single(); if (error) throw new TRPCError({ code: "BAD_REQUEST", message: error.message.slice(0,300) }); await recordAudit(actor, "automation.created", "automation", data.id, { platform: input.platform, post_id: input.post_id }); return data; }),
    update: protectedProcedure.input(z.object({ id: z.string().uuid(), keyword: z.string().trim().max(500).optional(), keywords: z.string().trim().max(500).optional(), any_comment: z.boolean().optional(), dm_template: z.string().trim().min(1).max(1000).optional(), button_text: z.string().trim().max(30).optional(), button_url: z.string().url().max(500).optional(), follow_required: z.boolean().optional(), is_active: z.boolean().optional() })).mutation(async ({ ctx, input }) => { const actor = await actorFor(ctx); assertRole(actor, ["admin", "editor"]); const { id, ...patch } = input; const clean: any = {}; if (patch.keywords !== undefined || patch.keyword !== undefined) { const kw = (patch.keywords || patch.keyword || "").toLowerCase().trim(); clean.keywords = kw; clean.keyword = kw.split(",")[0]?.trim() || kw; clean.any_comment = patch.any_comment ?? (!kw); } if (patch.any_comment !== undefined) clean.any_comment = patch.any_comment; if (patch.dm_template !== undefined) clean.dm_template = patch.dm_template; if (patch.button_text !== undefined) clean.button_text = patch.button_text; if (patch.button_url !== undefined) clean.button_url = patch.button_url; if (patch.follow_required !== undefined) clean.follow_required = patch.follow_required; if (patch.is_active !== undefined) clean.is_active = patch.is_active; clean.updated_at = new Date().toISOString(); const { data, error } = await getSupabase().from("automations").update(clean).eq("id", id).eq("organization_id", actor.organizationId).eq("site_id", actor.siteId).select("*").single(); if (error || !data) throw new TRPCError({ code: "NOT_FOUND", message: "Automation not found." }); return data; }),
