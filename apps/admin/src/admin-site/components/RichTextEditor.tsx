...
import {
  ...
  UploadCloud, WandSparkles, Youtube as YoutubeIcon,
} from "lucide-react";
...
export function RichTextEditor({ postId, title, initialContent, onChange }: { postId?: string; title: string; initialContent?: EditorValue | null; onChange: (value: { json: EditorValue; html: string }) => void }) {
  ...
  const uploadAudio = trpc.studio.media.upload.useMutation({ onSuccess: asset => { if (!asset.mime_type.startsWith("audio/")) { toast.info("That file is not an audio file."); return; } editor?.chain().focus().insertContent({ type: "audio", attrs: { src: asset.url, type: asset.mime_type } }).run(); setAudioOpen(false); setAudioSrc(""); toast.success("Audio player embedded."); }, onError: error => toast.error(error.message) });
  ...
  const addLink = () => { setLinkHref(""); setLinkOpen(true); };
  const applyLink = () => { const href = linkHref.trim(); if (!/^https?:\/\//i.test(href)) { toast.error("Please use an http or https URL."); return; } editor?.chain().focus().extendMarkRange("link").setLink({ href }).run(); setLinkOpen(false); setLinkHref(""); };
  ...
  const onAudioFile = async (file?: File) => { if (!file) return; const allowed = ["audio/mpeg", "audio/wav", "audio/mp4", "audio/ogg", "audio/webm"]; if (!allowed.includes(file.type)) { toast.error("Choose an MP3, WAV, M4A/AAC, OGG, or WebM audio file."); return; } if (file.size > 10 * 1024 * 1024) { toast.error("Audio files must be 10 MB or smaller."); return; } const base64 = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = () => reject(new Error("Could not read the selected file.")); reader.readAsDataURL(file); }); uploadAudio.mutate({ filename: file.name, mimeType: file.type as "audio/mpeg" | "audio/wav" | "audio/mp4" | "audio/ogg" | "audio/webm", base64, folder: "editor-audio", altText: "" }); };
  ...
  const insertVideo = () => { setVideoUrl(""); setVideoOpen(true); };
  const applyVideo = () => { const url = videoUrl.trim(); if (!url) { toast.error("Enter a YouTube or Vimeo URL."); return; } try { editor?.chain().focus().setYoutubeVideo({ src: url }).run(); setVideoOpen(false); setVideoUrl(""); toast.success("Video embedded."); } catch { toast.error("That video URL could not be embedded. Use a YouTube or Vimeo link."); } };
  ...
  return <div className="overflow-hidden rounded-xl border border-border bg-white shadow-sm">
    ...
    <Dialog open={audioOpen} onOpenChange={setAudioOpen}><DialogContent className="max-w-md"><DialogHeader><DialogTitle className="font-display text-2xl">Audio player</DialogTitle><DialogDescription>Upload an audio file, or embed a song, podcast clip, or audio note using a direct media URL.</DialogDescription></DialogHeader><div className="space-y-4"><div><Label>Upload an audio file</Label><input ref={audioFileRef} type="file" accept="audio/mpeg,audio/wav,audio/mp4,audio/ogg,audio/webm" className="hidden" onChange={event => { onAudioFile(event.target.files?.[0]); event.currentTarget.value = ""; }} /><Button type="button" variant="outline" className="mt-1.5 gap-2" disabled={uploadAudio.isPending} onClick={() => audioFileRef.current?.click()}>{uploadAudio.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <UploadCloud className="h-4 w-4" />}{uploadAudio.isPending ? "Uploading…" : "Upload audio"}</Button></div><div><Label>Or paste an audio URL</Label><Input value={audioSrc} onChange={event => setAudioSrc(event.target.value)} placeholder="https://…/audio.mp3" className="mt-1.5" /></div><div><Label>Format</Label><Select value={audioType} onValueChange={setAudioType}><SelectTrigger className="mt-1.5"><SelectValue /></SelectTrigger><SelectContent>{AUDIO_TYPES.map(type => <SelectItem key={type.value} value={type.value}>{type.label}</SelectItem>)}</SelectContent></Select></div><div className="flex justify-end gap-2 pt-2"><Button variant="outline" onClick={() => setAudioOpen(false)}>Cancel</Button><Button onClick={insertAudio}>Add player</Button></div></div></DialogContent></Dialog>

    <Dialog open={linkOpen} onOpenChange={setLinkOpen}><DialogContent className="max-w-md"><DialogHeader><DialogTitle className="font-display text-2xl">Add a link</DialogTitle><DialogDescription>Link the selected text to a secure page or another story.</DialogDescription></DialogHeader><div className="space-y-4"><div><Label>URL</Label><Input value={linkHref} onChange={event => setLinkHref(event.target.value)} placeholder="https://…" className="mt-1.5" /></div><div className="flex justify-end gap-2 pt-2"><Button variant="outline" onClick={() => setLinkOpen(false)}>Cancel</Button><Button onClick={applyLink}>Add link</Button></div></div></DialogContent></Dialog>

    <Dialog open={videoOpen} onOpenChange={setVideoOpen}><DialogContent className="max-w-md"><DialogHeader><DialogTitle className="font-display text-2xl">Embed a video</DialogTitle><DialogDescription>Paste a YouTube or Vimeo URL to embed a player.</DialogDescription></DialogHeader><div className="space-y-4"><div><Label>Video URL</Label><Input value={videoUrl} onChange={event => setVideoUrl(event.target.value)} placeholder="https://www.youtube.com/watch?v=…" className="mt-1.5" /></div><div className="flex justify-end gap-2 pt-2"><Button variant="outline" onClick={() => setVideoOpen(false)}>Cancel</Button><Button onClick={applyVideo}>Embed video</Button></div></div></DialogContent></Dialog>
