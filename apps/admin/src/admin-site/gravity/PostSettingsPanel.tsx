...
export function PostSettingsPanel({ draft, setDraft, onToggleId, postId, canPublish, status, featured, onToggleFeatured, featuredAsset, scheduledAt, setScheduledAt, onSchedule, scheduledAtDb, schedulePending, revisions, role, onRestore }: PostSettingsPanelProps) {
  ...
  const coverUrl = featuredAsset?.url || previewUrl;
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-xl border border-border bg-white p-5 text-foreground shadow-sm"><p className="font-label text-[10px] text-primary">Media</p><div className="mt-4"><Label className="text-xs text-muted-foreground">Featured media (thumbnail)</Label><div className="mt-1.5 flex flex-wrap items-center gap-2">{coverUrl ? <img src={coverUrl} alt="" className="h-12 w-12 rounded-md border border-border object-cover" /> : draft.featuredMediaId ? <div className="grid h-12 w-12 place-items-center rounded-md border border-border bg-muted text-[10px] text-muted-foreground">IMG</div> : null}<MediaUploadButton accept="image/jpeg,image/png,image/webp,image/gif" folder="featured" label={draft.featuredMediaId ? "Replace cover" : "Upload cover"} onUploaded={asset => { setPreviewUrl(asset.url); setDraft(current => ({ ...current, featuredMediaId: asset.id, ogImageUrl: current.ogImageUrl || asset.url })); }} />{draft.featuredMediaId && <Button type="button" variant="outline" size="sm" onClick={() => { setPreviewUrl(null); setDraft(current => ({ ...current, featuredMediaId: null })); }}>Remove</Button>}</div>{featuredAsset?.url && <p className="mt-1 text-xs text-muted-foreground">{featuredAsset.filename}</p>}</div></section>
      {postId && canPublish && <section className="rounded-xl border border-border bg-white p-5 text-foreground shadow-sm"><p className="font-label text-[10px] text-primary">Featured & schedule</p><label className="mt-4 flex items-center gap-2 text-sm"><Checkbox checked={featured} onCheckedChange={checked => onToggleFeatured(checked === true)} /><Star className="h-4 w-4 text-primary" />Feature on the homepage</label>{["draft", "review"].includes(status) && <div className="mt-4"><Label className="text-xs text-muted-foreground">Schedule publication</Label><input type="datetime-local" value={scheduledAt} onChange={event => setScheduledAt(event.target.value)} className="mt-1.5 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" /><div className="mt-2 flex gap-2"><Button size="sm" disabled={schedulePending || !scheduledAt} onClick={() => onSchedule({ scheduledAt: new Date(scheduledAt).toISOString() })}><CalendarClock className="mr-1 h-3.5 w-3.5" />Schedule</Button>{scheduledAtDb && <Button size="sm" variant="outline" disabled={schedulePending} onClick={() => { setScheduledAt(""); onSchedule({ clear: true }); }}>Clear</Button>}</div></div>}</section>}
      <section className="rounded-xl border border-border bg-white p-5 text-foreground shadow-sm"><p className="font-label text-[10px] text-primary">SEO & social</p><div className="mt-4 space-y-3"><div><Label className="text-xs text-muted-foreground">Meta title</Label><Input value={draft.metaTitle} onChange={event => setDraft({ ...draft, metaTitle: event.target.value })} className="mt-1" /><SeoHint value={draft.metaTitle} max={60} ideal="keep the page title under 60 characters so it is not cut off in search results" /></div><div><Label className="text-xs text-muted-foreground">Meta description</Label><Textarea value={draft.metaDescription} onChange={event => setDraft({ ...draft, metaDescription: event.target.value })} rows={3} className="mt-1" /><SeoHint value={draft.metaDescription} min={120} max={160} ideal="write it like ad copy — 150–160 characters shows fully in Google" /></div><div><Label className="text-xs text-muted-foreground">Canonical URL</Label><Input value={draft.canonicalUrl} onChange={event => setDraft({ ...draft, canonicalUrl: event.target.value })} placeholder="https://…" className="mt-1" /></div><div><Label className="text-xs text-muted-foreground">OG image URL</Label><Input value={draft.ogImageUrl} onChange={event => setDraft({ ...draft, ogImageUrl: event.target.value })} placeholder="https://…" className="mt-1" /></div></div></section>
