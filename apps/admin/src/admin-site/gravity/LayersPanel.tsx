...
import { ChevronRight, FileText, Image as ImageIcon, Music2, Video, X } from "lucide-react";
...
export function LayersPanel() {
  ...
  const Icon = ({ block }: { block: (typeof blocks)[number] }) => block.type === "image" ? <ImageIcon className="h-3 w-3" /> : block.type === "video" ? <Video className="h-3 w-3" /> : block.type === "audio" ? <Music2 className="h-3 w-3" /> : <FileText className="h-3 w-3" />;
  const label = (block: (typeof blocks)[number]) => (block.type === "text" ? (block.content || "Text").slice(0, 28) : block.caption || block.url || (block.type === "image" ? "Image" : block.type === "audio" ? "Audio" : "Video"));
