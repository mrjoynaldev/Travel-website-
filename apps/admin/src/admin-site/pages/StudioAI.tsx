...
export function StudioAIProviders() {
  ...
  const createProvider = trpc.llm.providers.create.useMutation({
    onSuccess: data => {
      providers.refetch();
      setModelsCache(prev => ({ ...prev, [data.provider.id]: data.models }));
      toast.success(
        `${data.provider.name} connected with ${data.models.length} models.`
      );
    },
    onError: error => toast.error(error.message),
  });
  const updateProvider = trpc.llm.providers.update.useMutation({
    onSuccess: () => {
      providers.refetch();
  ...
  const removeProvider = trpc.llm.providers.remove.useMutation({
    onSuccess: () => {
      providers.refetch();
      tasks.refetch();
  ...
  const setTask = trpc.llm.tasks.set.useMutation({
    onSuccess: () => {
      tasks.refetch();
      toast.success("Task routing saved.");
    },
    onError: error => toast.error(error.message),
  });
  const testProvider = trpc.llm.providers.test.useMutation({
    onSuccess: result => {
      if (result.ok) toast.success(`Provider replied: ${result.content}`);
      else toast.error(`Provider test failed: ${result.content}`);
    },
    onError: error => toast.error(error.message),
  });
  ...
  const modelsQuery = trpc.llm.providers.models.useQuery(
    { providerId: modelsTarget ?? "" },
    { enabled: false }
  );
  ...
  useEffect(() => {
    if (modelsTarget) {
      modelsQuery.refetch().then(result => {
        if (result.data)
          setModelsCache(prev => ({ ...prev, [modelsTarget]: result.data }));
      });
    }
  ...
  useEffect(() => {
    if (tasks.data) {
      ...
      for (const row of tasks.data) {
        if (row.providerId && !modelsCache[row.providerId])
          setModelsTarget(row.providerId);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tasks.data]);
  ...
  const submitProvider = () => {
    ...
    createProvider.mutate({
      providerType: form.providerType as
        | "openai"
        | "anthropic"
        | "gemini"
        | "nvidia"
        ...
        <header className="mb-5 flex flex-col gap-4 border-b border-border pb-5 sm:mb-7 sm:flex-row sm:items-end sm:justify-between sm:pb-6">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
              Add any provider's API key and its available models are discovered
              automatically. Route each editorial task to the model you choose.
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                OpenAI, Anthropic, Gemini, Nvidia NIM, or any OpenAI-compatible
                endpoint.
              </p>
            </div>
          </div>
        ...
        <section className="mt-7 overflow-hidden rounded-xl border border-border bg-white shadow-sm">
          ...
          {providers.isLoading ? (
            ...
            providers.data.map(provider => (
              <div
                key={provider.id}
                className="flex flex-col gap-3 border-b border-border p-5 last:border-b-0 md:flex-row md:items-center md:justify-between"
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`h-2.5 w-2.5 rounded-full ${provider.is_active ? "bg-emerald-500" : "bg-stone-300"}`}
                  />
                  <div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (
                        window.confirm(
                          `Remove ${provider.name}? Task routing that uses it will be cleared.`
                        )
                      )
                        removeProvider.mutate({ id: provider.id });
                    }}
                  >
            ...
            <p className="p-12 text-center text-sm text-muted-foreground">
              No providers connected. Add one above, or set OPENAI_API_KEY in
              the server environment as a fallback.
            </p>
          )}
        </section>
        ...
        <section className="mt-7 overflow-hidden rounded-xl border border-border bg-white shadow-sm">
          <div className="flex flex-col gap-2 border-b border-border px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Choose which provider model handles each editorial task.
              </p>
            </div>
            {modelsTarget && (
            ...
            {taskLabels &&
              Object.entries(taskLabels).map(([task, label]) => {
                ...
                return (
                  <div
                    key={task}
                    className="grid gap-3 p-5 lg:grid-cols-[1fr_220px_1fr_auto] lg:items-center"
                  >
                    <div>
                      <p className="font-medium">{label}</p>
                      <p className="mt-1 text-xs text-muted-foreground">
                        {current?.model
                          ? `${current.model}${current.providerId ? "" : ""}`
                          : "No model assigned — falls back to OPENAI_API_KEY"}
                      </p>
                    </div>
                    <Select
                      value={providerId}
                    ...
                    <Select
                      ...
                      onValueChange={value =>
                        setSelection(prev => ({
                          ...prev,
                          [task]: {
                            providerId: prev[task]?.providerId || "",
                            model: value,
                          },
                        }))
                      }
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        variant="outline"
                        className="gap-1.5"
                        disabled={!providerId || !current?.model}
                        onClick={() =>
                          testProvider.mutate({
                            providerId,
                            model: current!.model,
                      ...
                      <Button
                        ...
                        className="gap-1.5"
                        disabled={
                          !providerId || !current?.model || setTask.isPending
                        }
                        onClick={() =>
                          setTask.mutate({
                            task: task as
